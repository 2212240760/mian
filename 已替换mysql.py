...
def init_database():
    ...
    if conn:
        ...
        CREATE TABLE IF NOT EXISTS assessments (
            ...
            professional_skill INT,
            index_mastery INT,
            management_execution INT,
            communication_coordination INT,
            marketing_ability INT,
...
def get_leader_by_name(name):
    ...
    if conn:
        cursor = conn.cursor(dictionary=True)
        cursor.execute('SELECT * FROM grid_leaders WHERE name = %s', (name,))
...
def get_leader_assessments(leader_id):
    ...
    if conn:
        cursor = conn.cursor(dictionary=True)
        cursor.execute(
            'SELECT * FROM assessments WHERE leader_id = %s ORDER BY date DESC',
            (leader_id,)
        )
...
def add_leader(name, area):
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute('INSERT INTO grid_leaders (name, area) VALUES (%s, %s)', (name, area))
        conn.commit()
        conn.close()
